...
from data_provider import data_provider_periodic_nonperiodic
...
if need_train:
    # get data
    train_data, train_loader = data_provider_periodic_nonperiodic(configs, flag='train')
    valid_data, valid_loader = data_provider_periodic_nonperiodic(configs, flag='val')
    ...
    # for epoch in range(configs.train_epochs):
        ...
        for i, (seq_exogenous_periodic_x, 
                seq_exogenous_non_periodic_x, 
                seq_endogenous_periodic_x, 
                seq_endogenous_non_periodic_x,
                seq_endogenous_y, 
                seq_x_mark, 
                seq_y_mark, x_time_points, y_time_points, x_time, y_time) in enumerate(train_loader):
            iter_count += 1
            model_optim.zero_grad()

            # seq_exogenous_x = seq_exogenous_x.float().to(device)
            seq_exogenous_periodic_x = seq_exogenous_periodic_x.float().to(device)
            seq_exogenous_non_periodic_x = seq_exogenous_non_periodic_x.float().to(device)
            seq_endogenous_periodic_x = seq_endogenous_periodic_x.float().to(device)
            seq_endogenous_non_periodic_x = seq_endogenous_non_periodic_x.float().to(device)
            ...
            if configs.use_amp:
                with torch.cuda.amp.autocast():
                    if configs.output_attention:
                        outputs = model(seq_exogenous_periodic_x, seq_exogenous_non_periodic_x, seq_endogenous_periodic_x, seq_endogenous_non_periodic_x, seq_x_mark, x_time, y_time)[0]
                    else:
                        outputs = model(seq_exogenous_periodic_x, seq_exogenous_non_periodic_x, seq_endogenous_periodic_x, seq_endogenous_non_periodic_x, seq_x_mark, x_time, y_time)
        ...
        trues = []
        with torch.no_grad():
            for i, (seq_exogenous_periodic_x, seq_exogenous_non_periodic_x, seq_endogenous_periodic_x, seq_endogenous_non_periodic_x, seq_endogenous_y, seq_x_mark, seq_y_mark, x_time_points, y_time_points, x_time, y_time) in enumerate(valid_loader):
        ...
        # print(f"实际训练中的参数量: {total_trainable_params}")
        # sys.exit(0)
...
if need_test:
    ...
    test_data, test_loader = data_provider_periodic_nonperiodic(configs, flag='test')
    if need_load:
        print('loading model...')
    ...
    model.eval()
    with torch.no_grad():
        for i, (seq_exogenous_periodic_x, seq_exogenous_non_periodic_x, seq_endogenous_periodic_x, seq_endogenous_non_periodic_x, seq_endogenous_y, seq_x_mark, seq_y_mark, x_time_points, y_time_points, x_time, y_time) in tqdm.tqdm(enumerate(test_loader)):
            # seq_exogenous_x = seq_exogenous_x.float().to(device)
            seq_exogenous_periodic_x = seq_exogenous_periodic_x.float().to(device)
            seq_exogenous_non_periodic_x = seq_exogenous_non_periodic_x.float().to(device)
            seq_endogenous_periodic_x = seq_endogenous_periodic_x.float().to(device)
            ...
            #     mask = merged['日前风电光伏总负荷'].values > 18500
            #     merged.loc[mask, 'pred'] = 25
            #     # 6. 恢复为 (1, 1080, 1) 的 numpy 数组
            #     pred_original = merged['pred'].values.reshape(1, -1, 1)


            # print(type(true_original))
            # print(true_original.shape)
            # print(type(y_time_points))
            # print(len(y_time_points))
            # sys.exit(0)
            ...
            if configs.check_data == True and configs.batch_size == 1:
                if test_data.scale and configs.inverse:
                    ...
                    shape_expx = seq_exogenous_periodic_x.shape
                    shape_exnpx = seq_exogenous_non_periodic_x.shape
                    seq_endogenous_x = seq_endogenous_x.detach().cpu().numpy()
                    # seq_exogenous_x = seq_exogenous_x.detach().cpu().numpy()
                    seq_exogenous_periodic_x = seq_exogenous_periodic_x.detach().cpu().numpy()
                    seq_exogenous_non_periodic_x = seq_exogenous_non_periodic_x.detach().cpu().numpy()
                    ...
                    seq_exogenous_periodic_x_inversed = test_data.inverse_exogenous_periodic_transform(seq_exogenous_periodic_x.squeeze(0)).reshape(shape_expx)
                    seq_exogenous_non_periodic_x_inversed = test_data.inverse_exogenous_non_periodic_transform(seq_exogenous_non_periodic_x.squeeze(0)).reshape(shape_exnpx)
                    print(f"batch_size: {i}, train_others: {seq_exogenous_periodic_x_inversed, seq_exogenous_non_periodic_x_inversed} train_target: {seq_endogenous_x_inversed} true_target {seq_endogenous_y_inversed}")
    ...
    # print(preds_original.shape)
    # print(trues_original.shape)
    # sys.exit(0)
    ...
    f.write(f'Model Param Num: {total_trainable_params:,}\n')
    if configs.train_alpha:
    ...
    f.write('mse: {}\nmae: {}\nrmse: {}\nmape: {}\nmspe: {}\nacc: {}\ntime_segmented_acc: {}'.format(mse, mae, rmse, mape, mspe, acc, time_segmented_acc))
    f.write('\n')
    # print(type(time_segmented_acc))
    # exit(0)
    f.write(f'{acc}\n{time_segmented_acc["6:00-10:00"]}\n{time_segmented_acc["11:00-16:00"]}\n{time_segmented_acc["17:00-21:00"]}\n')
    f.write('\n')
    f.close()
